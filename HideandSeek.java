...
public class HideandSeek {
	...
	public static void main(String[] args) {
		...
		do {
			...
			map[number1][number2] = girl; // number1 girs room number2 box
			girl.setRoomNumber(number1);
			map[number3][0] = mother; // number3 moms room number
			girl.setRoomNumber(number1);
					...
					for (int j = 1; j < 6; j++) {
						...
						for (int j = 1; j < 6; j++) {
							if (girl.getRoomNumber() == i && number2 == j) {
								hp = mother.getRoomNumber() * 5
										+ (girl.getRoomNumber() - mother.getRoomNumber()) * 2 * 5 + number2 * 2;
								time += i * 5 + j;
								cry++;
								continue;
